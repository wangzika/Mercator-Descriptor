...
def get_gt_sens_poses10(fpath_gt_sens_poses):
    """
    :return: 2d matrix, each row is a 12 dim elements
    """
...
def get_points_ours2(fp_gt_sens_poses, fp_outcome):
    ...
    #每10个
    gt_pose10 = get_gt_sens_poses10(fp_gt_sens_poses)  # the sensor poses must be ordered by time/creation/acquisition
    gt_positive = np.zeros(gt_pose.shape[0])
    gt_points = gt_pose[:, [3, 7, 11]]
    gt_points10 = gt_pose[:, [3, 7, 11]]

    tree = KDTree(gt_points)

    for i in range(gt_pose.shape[0]):
        # if i!=0 and i%10==0:
        near_points = tree.query_ball_point(gt_points10[i, :], thres_dist)
...
if __name__ == "__main__":
    file_gt_sens_poses = "/home/xdh/下载/dataset/poses/00.txt"
    file_outcome = "/home/xdh/SLAM/std_ws/src/STD/res_out/test/wzb_deepm_60_res-kitti00_generated.txt"
